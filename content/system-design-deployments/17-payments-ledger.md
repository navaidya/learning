---
title: "AI-Native Payments Ledger Deployment Architecture"
summary: "A cloud-neutral deployment using restricted regional ledger cell with fenced disaster recovery for Payments ledger, with explicit failure domains, state ownership, recovery, and AI degradation."
systemDesign: 17-payments-ledger
order: 18
deploymentStyle: "Restricted regional ledger cell with fenced disaster recovery"
availabilityTarget: "99.99% ledger read and 99.95% write availability with zero acknowledged ledger loss"
regions: multi-region
tags: [ledger, payments, restricted-zone, reconciliation]
---
...
The deployment protects the parent design's critical invariant—double-entry balance, idempotency, monotonic status, and no duplicate external capture—while meeting 99.99% ledger read and 99.95% write availability with zero acknowledged ledger loss. The synchronous customer path remains short; derived views, external calls, analytics, and expensive inference move behind durable boundaries whenever the product contract permits.

Assume three independent failure zones in an active region, immutable artifacts, workload identity, managed or dedicated state services separated from stateless request compute, and a paired recovery region. A licensed processor handles regulated card rails and financial correctness outranks immediate success. The Kubernetes or compute API is a management plane and never carries customer requests.

## 2. Traffic classes and critical paths

| Traffic class | Latency or freshness objective | Consistency | Deployment implication |
|---|---|---|---|
| Critical command | payment command p99 below 1 second excluding external challenge flows | Enforce the product invariant before acknowledgement | Ownership-route to zonal service replicas and authoritative regional state. |
...
| Event and background work | Seconds to minutes by business priority | At-least-once with idempotent consumers | Partition Transactional outbox and reconciliation streams and isolate critical from bulk queues. |
...
```mermaid
sequenceDiagram
  accTitle: Payments ledger critical deployed request path
  ...
  participant S as Idempotency, risk rules, payment orchestration, and ledger services
  participant D as Serializable double-entry ledger database
  participant Q as Transactional outbox and reconciliation streams
  participant W as Processor, settlement, dispute, and reconciliation workers
  participant A as Deadline-bounded advisory fraud scoring
...
```mermaid
flowchart TB
  accTitle: Payments ledger cloud-neutral deployment
  ...
  Client["Clients and operators<br/>Issue product commands and reads"] --> Edge["DDoS, WAF, mTLS, and payment ingress<br/>Terminates public traffic and rejects abuse"]
  ...
  subgraph Active["Active serving region"]
    LB["Restricted regional payment API gateway<br/>Distributes traffic across healthy zones"]
    CP["Managed compute control plane<br/>Schedules and manages workloads only"]
    subgraph Compute["Isolated payment orchestration cluster"]
      ZA["Zone A replicas<br/>Gateway and Idempotency, risk rules, payment orchestration, and ledger services"]
      ZB["Zone B replicas<br/>Gateway and Idempotency, risk rules, payment orchestration, and ledger services"]
      ZC["Zone C replicas<br/>Gateway and Idempotency, risk rules, payment orchestration, and ledger services"]
      Worker["Processor, settlement, dispute, and reconciliation workers<br/>Consumes durable retryable work"]
      AI["Deadline-bounded advisory fraud scoring<br/>Deadline-bounded and policy-controlled"]
    ...
    subgraph State["Managed or dedicated state plane outside request compute"]
      Primary[("Serializable double-entry ledger database<br/>Owns authoritative durable state")]
      Fast[("Read replicas and bounded reference-data caches<br/>Serves low-latency derived or ephemeral state")]
      Stream[("Transactional outbox and reconciliation streams<br/>Decouples committed asynchronous work")]
  ...
  Traffic --> LB
  Worker --> External["Payment processor, banking rails, and HSM<br/>Reached through restricted egress"]
...
|---|---|---|---|---|
| DDoS, WAF, mTLS, and payment ingress | Globally or regionally distributed edge | Edge location and policy | Routing/config only | Rejects attacks and routes only to explicitly healthy regions. |
| Restricted regional payment API gateway | Active region across three zones | Managed capacity or gateway replica | No | Drains failed zones and preserves connection or request retries. |
...
| Idempotency, risk rules, payment orchestration, and ledger services | Private zonal replicas in Isolated payment orchestration cluster | Replica by CPU, concurrency, connections, or queue lag | No local authority | Losing one replica or zone removes endpoints without losing durable state. |
| Serializable double-entry ledger database | Managed or dedicated multi-zone state plane | Shard, partition, connection, and storage unit | Yes | Quorum or replica failover preserves acknowledged authoritative state. |
| Read replicas and bounded reference-data caches | Regional replicas or immutable serving indexes | Shard and memory/index bytes | Derived or ephemeral | Rebuilds from authority; stale data is bounded or bypassed for policy changes. |
| Transactional outbox and reconciliation streams | Multi-zone partitioned durable log/queue | Partition throughput and retention | Yes | Consumers replay from checkpoints after worker failure. |
| Processor, settlement, dispute, and reconciliation workers | Private autoscaled worker pools | Queue age, lag, and concurrency | Checkpoints only | Work is retried idempotently or isolated in a dead-letter workflow. |
| Deadline-bounded advisory fraud scoring | Isolated CPU/accelerator or managed inference pool | Requests, tokens, batches, and accelerator utilization | Model artifacts | Circuit opens to a deterministic fallback without failing the product. |
...
DDoS, WAF, mTLS, and payment ingress terminates public TLS, applies DDoS/WAF controls, rate limits abuse, and strips untrusted forwarding metadata. Traffic policy chooses a region only after independent health and ownership checks. At Restricted regional payment API gateway, the platform authenticates or validates signed identity, attaches trace and idempotency context, enforces request-size and deadline policy, and distributes traffic across healthy zones.
...
Place Isolated payment orchestration cluster across three zones with topology spread, disruption budgets, anti-affinity for critical replicas, and reserved capacity so any one zone can disappear. Run Idempotency, risk rules, payment orchestration, and ledger services as stateless deployments where the domain permits; give Processor, settlement, dispute, and reconciliation workers separate queues and resource pools so backlogs cannot exhaust request capacity. Specialized inference is isolated from product compute.
...
Treat Serializable double-entry ledger database as authoritative. Partition by merchant/account ownership and transaction ID while preserving balanced journal transactions. Apply strong consistency only to double-entry balance, idempotency, monotonic status, and no duplicate external capture; allow bounded eventual consistency for derived caches, indexes, analytics, and model features. Give Read replicas and bounded reference-data caches a rebuild path and version watermark so stale values cannot overrule newer security or deletion state.

Begin Transactional outbox and reconciliation streams after a durable commit, preferably through a transactional outbox or native log append. Consumers checkpoint offsets, deduplicate event IDs, quarantine poison records, and expose oldest-work age. Archives, snapshots, and model artifacts use encrypted versioned object storage with lifecycle policy.

## 7. Network zones and security boundaries

Separate public edge, private application, restricted state, management, telemetry, and controlled-egress zones. Default-deny network policy allows only named service identities and ports. Workload identity replaces static credentials; secrets and envelope keys rotate through managed key services. isolate card data, use HSM-backed keys, dual control, tamper-evident audit, and strict egress.

Payment processor, banking rails, and HSM is reached through allowlisted egress proxies with deadlines, circuit breakers, response-size limits, and audit. Treat external and model output as untrusted data. Administrative and break-glass actions require strong authentication, least privilege, short sessions, immutable audit, and where consequence demands it, dual approval.
...
The sizing method is explicit: payment commands/s size API and ledger connections; external latency, reconciliation files, and exception rate independently size worker pools. Load-test the complete protocol and record shape rather than relying on vendor headline throughput. Keep at least 30% burst/rebalance headroom plus enough reserved capacity to survive the largest declared failure domain.
...
The assumed **RPO** is zero for acknowledged ledger entries and idempotency records. The assumed **RTO** is under 30 minutes for fenced ledger-region promotion with processor reconciliation before full writes. These are product choices, not properties automatically obtained by creating a second cluster. Backups are encrypted, immutable where required, restored in drills, and checked against application-level invariants.
...
|---|---|---|---|
| Failure: the ledger primary fails | quorum and transaction errors | promote a synchronous replica and re-resolve connections | idempotent commands retry without duplicate posting |
| Failure: a processor times out | deadline and circuit state | record pending state, stop blind retries, and reconcile asynchronously | the user sees pending rather than a duplicate charge |
| Failure: fraud inference fails | deadline or model guardrail | use deterministic hard-rule fallback and route uncertain cases to review | money movement never depends solely on a model |

AI degradation is explicit: apply deterministic velocity, amount, identity, and allow/deny rules when fraud inference is unavailable. The **deterministic product fallback** is tested in canaries and game days; inference may improve ranking, prediction, classification, or assistance but cannot bypass identity, authorization, ownership, money, privacy, durability, or safety rules.
...
1. Start at DDoS, WAF, mTLS, and payment ingress: explain trust, routing, and what can be served safely at the edge.
2. Follow the solid request path through Restricted regional payment API gateway into zonal Idempotency, risk rules, payment orchestration, and ledger services replicas.
3. Identify Serializable double-entry ledger database as authoritative and state the partition key: merchant/account ownership and transaction ID while preserving balanced journal transactions.
4. Draw Read replicas and bounded reference-data caches and Transactional outbox and reconciliation streams only after explaining which reads and side effects may be derived or eventual.
5. Separate Processor, settlement, dispute, and reconciliation workers, Deadline-bounded advisory fraud scoring, and Payment processor, banking rails, and HSM from the authoritative transaction with deadlines and durable events.
...
| Authoritative state | Serializable double-entry ledger database | Managed relational, distributed SQL, key-value, or dedicated state fleet | Transactions, quorum, latency, partitioning, restore behavior. |
| Fast serving state | Read replicas and bounded reference-data caches | In-memory cache, read replica, search/vector index | Staleness, invalidation, hot-key behavior, rebuild time. |
| Durable messaging | Transactional outbox and reconciliation streams | Partitioned log, managed queue, transactional outbox relay | Ordering, replay, retention, throughput, consumer isolation. |
...
| AI/model serving | Deadline-bounded advisory fraud scoring | In-cluster serving or managed inference endpoint | Tail latency, policy, model routing, cost, deterministic fallback. |
