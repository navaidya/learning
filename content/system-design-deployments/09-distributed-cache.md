---
title: "AI-Native Distributed Cache Deployment Architecture"
summary: "A cloud-neutral deployment using zonal shard fleet with quorum membership control for Distributed cache, with explicit failure domains, state ownership, recovery, and AI degradation."
systemDesign: 09-distributed-cache
order: 10
deploymentStyle: "Zonal shard fleet with quorum membership control"
availabilityTarget: "99.99% cache availability while one node or zone fails"
regions: multi-region
tags: [sharding, replication, membership, hot-keys]
---
...
The deployment protects the parent design's critical invariant—declared consistency and TTL semantics must hold across failover—while meeting 99.99% cache availability while one node or zone fails. The synchronous customer path remains short; derived views, external calls, analytics, and expensive inference move behind durable boundaries whenever the product contract permits.

Assume three independent failure zones in an active region, immutable artifacts, workload identity, managed or dedicated state services separated from stateless request compute, and a paired recovery region. Keys are tenant-namespaced, misses are acceptable, and applications own source-of-truth recovery. The Kubernetes or compute API is a management plane and never carries customer requests.

## 2. Traffic classes and critical paths

| Traffic class | Latency or freshness objective | Consistency | Deployment implication |
|---|---|---|---|
| Critical command | p99 get below 2 ms inside a region | Enforce the product invariant before acknowledgement | Ownership-route to zonal service replicas and authoritative regional state. |
...
| Event and background work | Seconds to minutes by business priority | At-least-once with idempotent consumers | Partition Membership and optional persistence log and isolate critical from bulk queues. |
...
```mermaid
sequenceDiagram
  accTitle: Distributed cache critical deployed read path
  ...
  participant F as Near-client cache and hot-key replicas
  participant S as Shard routing, eviction, replication, and membership
  participant D as In-memory primary shards with zonal replicas
  participant Q as Membership and optional persistence log
...
```mermaid
flowchart TB
  accTitle: Distributed cache cloud-neutral deployment
  ...
  Client["Clients and operators<br/>Issue product commands and reads"] --> Edge["Private service endpoint and tenant authentication<br/>Terminates public traffic and rejects abuse"]
  ...
  subgraph Active["Active serving region"]
    LB["Cache routing proxies or smart clients<br/>Distributes traffic across healthy zones"]
    CP["Managed compute control plane<br/>Schedules and manages workloads only"]
    subgraph Compute["Dedicated stateful cache node fleet"]
      ZA["Zone A replicas<br/>Gateway and Shard routing, eviction, replication, and membership"]
      ZB["Zone B replicas<br/>Gateway and Shard routing, eviction, replication, and membership"]
      ZC["Zone C replicas<br/>Gateway and Shard routing, eviction, replication, and membership"]
      Worker["Rebalance, snapshot, repair, and hot-key workers<br/>Consumes durable retryable work"]
      AI["Advisory hot-key and capacity forecasting<br/>Deadline-bounded and policy-controlled"]
    ...
    subgraph State["Managed or dedicated state plane outside request compute"]
      Primary[("In-memory primary shards with zonal replicas<br/>Owns authoritative durable state")]
      Fast[("Near-client cache and hot-key replicas<br/>Serves low-latency derived or ephemeral state")]
      Stream[("Membership and optional persistence log<br/>Decouples committed asynchronous work")]
  ...
  Traffic --> LB
  Worker --> External["Source database and snapshot object storage<br/>Reached through restricted egress"]
...
|---|---|---|---|---|
| Private service endpoint and tenant authentication | Globally or regionally distributed edge | Edge location and policy | Routing/config only | Rejects attacks and routes only to explicitly healthy regions. |
| Cache routing proxies or smart clients | Active region across three zones | Managed capacity or gateway replica | No | Drains failed zones and preserves connection or request retries. |
...
| Shard routing, eviction, replication, and membership | Private zonal replicas in Dedicated stateful cache node fleet | Replica by CPU, concurrency, connections, or queue lag | No local authority | Losing one replica or zone removes endpoints without losing durable state. |
| In-memory primary shards with zonal replicas | Managed or dedicated multi-zone state plane | Shard, partition, connection, and storage unit | Yes | Quorum or replica failover preserves acknowledged authoritative state. |
| Near-client cache and hot-key replicas | Regional replicas or immutable serving indexes | Shard and memory/index bytes | Derived or ephemeral | Rebuilds from authority; stale data is bounded or bypassed for policy changes. |
| Membership and optional persistence log | Multi-zone partitioned durable log/queue | Partition throughput and retention | Yes | Consumers replay from checkpoints after worker failure. |
| Rebalance, snapshot, repair, and hot-key workers | Private autoscaled worker pools | Queue age, lag, and concurrency | Checkpoints only | Work is retried idempotently or isolated in a dead-letter workflow. |
| Advisory hot-key and capacity forecasting | Isolated CPU/accelerator or managed inference pool | Requests, tokens, batches, and accelerator utilization | Model artifacts | Circuit opens to a deterministic fallback without failing the product. |
...
Private service endpoint and tenant authentication terminates public TLS, applies DDoS/WAF controls, rate limits abuse, and strips untrusted forwarding metadata. Traffic policy chooses a region only after independent health and ownership checks. At Cache routing proxies or smart clients, the platform authenticates or validates signed identity, attaches trace and idempotency context, enforces request-size and deadline policy, and distributes traffic across healthy zones.
...
Place Dedicated stateful cache node fleet across three zones with topology spread, disruption budgets, anti-affinity for critical replicas, and reserved capacity so any one zone can disappear. Run Shard routing, eviction, replication, and membership as stateless deployments where the domain permits; give Rebalance, snapshot, repair, and hot-key workers separate queues and resource pools so backlogs cannot exhaust request capacity. Specialized inference is isolated from product compute.
...
Treat In-memory primary shards with zonal replicas as authoritative. Partition by consistent hash of tenant-qualified key. Apply strong consistency only to declared consistency and TTL semantics must hold across failover; allow bounded eventual consistency for derived caches, indexes, analytics, and model features. Give Near-client cache and hot-key replicas a rebuild path and version watermark so stale values cannot overrule newer security or deletion state.

Begin Membership and optional persistence log after a durable commit, preferably through a transactional outbox or native log append. Consumers checkpoint offsets, deduplicate event IDs, quarantine poison records, and expose oldest-work age. Archives, snapshots, and model artifacts use encrypted versioned object storage with lifecycle policy.

## 7. Network zones and security boundaries

Separate public edge, private application, restricted state, management, telemetry, and controlled-egress zones. Default-deny network policy allows only named service identities and ports. Workload identity replaces static credentials; secrets and envelope keys rotate through managed key services. authenticate tenants, encrypt transport, enforce quotas, and prevent cross-tenant key access.

Source database and snapshot object storage is reached through allowlisted egress proxies with deadlines, circuit breakers, response-size limits, and audit. Treat external and model output as untrusted data. Administrative and break-glass actions require strong authentication, least privilege, short sessions, immutable audit, and where consequence demands it, dual approval.
...
The sizing method is explicit: target operations/s per node, working-set bytes, replication factor, and one-zone-loss headroom determine shard and replica count. Load-test the complete protocol and record shape rather than relying on vendor headline throughput. Keep at least 30% burst/rebalance headroom plus enough reserved capacity to survive the largest declared failure domain.
...
The assumed **RPO** is cache-only data may be zero-value; persisted tiers choose seconds-to-minutes based on source rebuild cost. The assumed **RTO** is seconds for replica promotion and minutes for shard repair or snapshot restore. These are product choices, not properties automatically obtained by creating a second cluster. Backups are encrypted, immutable where required, restored in drills, and checked against application-level invariants.
...
|---|---|---|---|
| Failure: a cache node or zone fails | gossip, health probes, and replica lag | advance ownership epoch and promote an in-sync replica | clients retry the new owner |
| Failure: a hot key saturates one shard | per-key QPS and tail latency | replicate or request-coalesce that key and enforce tenant quotas | other shards remain isolated |
| Failure: capacity forecasting fails | model deadline or drift guardrail | use deterministic utilization and eviction thresholds | cache operations remain fully available |

AI degradation is explicit: use deterministic eviction, consistent hashing, and static hot-key thresholds when forecasts are unavailable. The **deterministic product fallback** is tested in canaries and game days; inference may improve ranking, prediction, classification, or assistance but cannot bypass identity, authorization, ownership, money, privacy, durability, or safety rules.
...
1. Start at Private service endpoint and tenant authentication: explain trust, routing, and what can be served safely at the edge.
2. Follow the solid request path through Cache routing proxies or smart clients into zonal Shard routing, eviction, replication, and membership replicas.
3. Identify In-memory primary shards with zonal replicas as authoritative and state the partition key: consistent hash of tenant-qualified key.
4. Draw Near-client cache and hot-key replicas and Membership and optional persistence log only after explaining which reads and side effects may be derived or eventual.
5. Separate Rebalance, snapshot, repair, and hot-key workers, Advisory hot-key and capacity forecasting, and Source database and snapshot object storage from the authoritative transaction with deadlines and durable events.
...
| Authoritative state | In-memory primary shards with zonal replicas | Managed relational, distributed SQL, key-value, or dedicated state fleet | Transactions, quorum, latency, partitioning, restore behavior. |
| Fast serving state | Near-client cache and hot-key replicas | In-memory cache, read replica, search/vector index | Staleness, invalidation, hot-key behavior, rebuild time. |
| Durable messaging | Membership and optional persistence log | Partitioned log, managed queue, transactional outbox relay | Ordering, replay, retention, throughput, consumer isolation. |
...
| AI/model serving | Advisory hot-key and capacity forecasting | In-cluster serving or managed inference endpoint | Tail latency, policy, model routing, cost, deterministic fallback. |
