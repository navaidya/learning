---
title: "AI-Native Social Network Deployment Architecture"
summary: "A cloud-neutral deployment using graph and content cells with privacy-filtered global reads for Social network, with explicit failure domains, state ownership, recovery, and AI degradation."
systemDesign: 06-ai-native-social-network
order: 7
deploymentStyle: "Graph and content cells with privacy-filtered global reads"
availabilityTarget: "99.99% read availability with durable user-authored content"
regions: global
tags: [social-graph, privacy, media, recommendations]
---
...
The deployment protects the parent design's critical invariant—privacy-policy enforcement must precede every feed, search, agent, and media response—while meeting 99.99% read availability with durable user-authored content. The synchronous customer path remains short; derived views, external calls, analytics, and expensive inference move behind durable boundaries whenever the product contract permits.

Assume three independent failure zones in an active region, immutable artifacts, workload identity, managed or dedicated state services separated from stateless request compute, and a paired recovery region. Graph and content are region-affine, media is immutable by version, and feeds allow bounded staleness. The Kubernetes or compute API is a management plane and never carries customer requests.

## 2. Traffic classes and critical paths

| Traffic class | Latency or freshness objective | Consistency | Deployment implication |
|---|---|---|---|
| Critical command | feed p99 below 400 ms and profile reads below 200 ms | Enforce the product invariant before acknowledgement | Ownership-route to zonal service replicas and authoritative regional state. |
...
| Event and background work | Seconds to minutes by business priority | At-least-once with idempotent consumers | Partition Graph, content, media, and policy events and isolate critical from bulk queues. |
...
```mermaid
sequenceDiagram
  accTitle: Social network critical deployed read path
  ...
  participant F as Privacy-aware feed, profile, and media caches
  participant S as Graph, content, privacy, and feed services
  participant D as Sharded graph and content databases
  participant Q as Graph, content, media, and policy events
  participant A as Recommendation and bounded personal-agent gateway
...
```mermaid
flowchart TB
  accTitle: Social network cloud-neutral deployment
  ...
  Client["Clients and operators<br/>Issue product commands and reads"] --> Edge["Global CDN, media edge, DDoS, and WAF<br/>Terminates public traffic and rejects abuse"]
  ...
  subgraph Active["Active serving region"]
    LB["Regional identity-aware API gateway<br/>Distributes traffic across healthy zones"]
    CP["Managed compute control plane<br/>Schedules and manages workloads only"]
    subgraph Compute["Graph and content application cells"]
      ZA["Zone A replicas<br/>Gateway and Graph, content, privacy, and feed services"]
      ZB["Zone B replicas<br/>Gateway and Graph, content, privacy, and feed services"]
      ZC["Zone C replicas<br/>Gateway and Graph, content, privacy, and feed services"]
      Worker["Fan-out, media, indexing, and deletion workers<br/>Consumes durable retryable work"]
      AI["Recommendation and bounded personal-agent gateway<br/>Deadline-bounded and policy-controlled"]
    ...
    subgraph State["Managed or dedicated state plane outside request compute"]
      Primary[("Sharded graph and content databases<br/>Owns authoritative durable state")]
      Fast[("Privacy-aware feed, profile, and media caches<br/>Serves low-latency derived or ephemeral state")]
      Stream[("Graph, content, media, and policy events<br/>Decouples committed asynchronous work")]
  ...
  Traffic --> LB
  Worker --> External["Identity recovery and media processing providers<br/>Reached through restricted egress"]
...
|---|---|---|---|---|
| Global CDN, media edge, DDoS, and WAF | Globally or regionally distributed edge | Edge location and policy | Routing/config only | Rejects attacks and routes only to explicitly healthy regions. |
| Regional identity-aware API gateway | Active region across three zones | Managed capacity or gateway replica | No | Drains failed zones and preserves connection or request retries. |
...
| Graph, content, privacy, and feed services | Private zonal replicas in Graph and content application cells | Replica by CPU, concurrency, connections, or queue lag | No local authority | Losing one replica or zone removes endpoints without losing durable state. |
| Sharded graph and content databases | Managed or dedicated multi-zone state plane | Shard, partition, connection, and storage unit | Yes | Quorum or replica failover preserves acknowledged authoritative state. |
| Privacy-aware feed, profile, and media caches | Regional replicas or immutable serving indexes | Shard and memory/index bytes | Derived or ephemeral | Rebuilds from authority; stale data is bounded or bypassed for policy changes. |
| Graph, content, media, and policy events | Multi-zone partitioned durable log/queue | Partition throughput and retention | Yes | Consumers replay from checkpoints after worker failure. |
| Fan-out, media, indexing, and deletion workers | Private autoscaled worker pools | Queue age, lag, and concurrency | Checkpoints only | Work is retried idempotently or isolated in a dead-letter workflow. |
| Recommendation and bounded personal-agent gateway | Isolated CPU/accelerator or managed inference pool | Requests, tokens, batches, and accelerator utilization | Model artifacts | Circuit opens to a deterministic fallback without failing the product. |
...
Global CDN, media edge, DDoS, and WAF terminates public TLS, applies DDoS/WAF controls, rate limits abuse, and strips untrusted forwarding metadata. Traffic policy chooses a region only after independent health and ownership checks. At Regional identity-aware API gateway, the platform authenticates or validates signed identity, attaches trace and idempotency context, enforces request-size and deadline policy, and distributes traffic across healthy zones.
...
Place Graph and content application cells across three zones with topology spread, disruption budgets, anti-affinity for critical replicas, and reserved capacity so any one zone can disappear. Run Graph, content, privacy, and feed services as stateless deployments where the domain permits; give Fan-out, media, indexing, and deletion workers separate queues and resource pools so backlogs cannot exhaust request capacity. Specialized inference is isolated from product compute.
...
Treat Sharded graph and content databases as authoritative. Partition by user home shard with replicated public read projections. Apply strong consistency only to privacy-policy enforcement must precede every feed, search, agent, and media response; allow bounded eventual consistency for derived caches, indexes, analytics, and model features. Give Privacy-aware feed, profile, and media caches a rebuild path and version watermark so stale values cannot overrule newer security or deletion state.

Begin Graph, content, media, and policy events after a durable commit, preferably through a transactional outbox or native log append. Consumers checkpoint offsets, deduplicate event IDs, quarantine poison records, and expose oldest-work age. Archives, snapshots, and model artifacts use encrypted versioned object storage with lifecycle policy.

## 7. Network zones and security boundaries

Separate public edge, private application, restricted state, management, telemetry, and controlled-egress zones. Default-deny network policy allows only named service identities and ports. Workload identity replaces static credentials; secrets and envelope keys rotate through managed key services. enforce consent-aware access, isolate personal-agent memory, and audit sensitive graph queries.

Identity recovery and media processing providers is reached through allowlisted egress proxies with deadlines, circuit breakers, response-size limits, and audit. Treat external and model output as untrusted data. Administrative and break-glass actions require strong authentication, least privilege, short sessions, immutable audit, and where consequence demands it, dual approval.
...
The sizing method is explicit: graph edge degree, feed fan-out, media bandwidth, and privacy-check QPS are measured separately and assigned isolated scaling pools. Load-test the complete protocol and record shape rather than relying on vendor headline throughput. Keep at least 30% burst/rebalance headroom plus enough reserved capacity to survive the largest declared failure domain.
...
The assumed **RPO** is zero for acknowledged graph, content, and privacy changes; minutes for derived recommendations. The assumed **RTO** is under 15 minutes for a user-cell failover and hours for full recommendation rebuild. These are product choices, not properties automatically obtained by creating a second cluster. Backups are encrypted, immutable where required, restored in drills, and checked against application-level invariants.
...
|---|---|---|---|
| Failure: a graph shard fails | quorum health and adjacency-read errors | promote a replica and invalidate affected graph caches | profile reads may retry while privacy remains enforced |
| Failure: privacy policy propagation lags | policy version watermark | fail closed for uncertain disclosures and bypass stale feed entries | some content is hidden rather than leaked |
| Failure: recommendation or agent inference fails | model deadline, budget, or policy block | use the deterministic followed-content fallback and disable agent tools | core social sharing remains available |

AI degradation is explicit: serve deterministic recent-followed content and disable personal-agent actions when models or policy context are unavailable. The **deterministic product fallback** is tested in canaries and game days; inference may improve ranking, prediction, classification, or assistance but cannot bypass identity, authorization, ownership, money, privacy, durability, or safety rules.
...
1. Start at Global CDN, media edge, DDoS, and WAF: explain trust, routing, and what can be served safely at the edge.
2. Follow the solid request path through Regional identity-aware API gateway into zonal Graph, content, privacy, and feed services replicas.
3. Identify Sharded graph and content databases as authoritative and state the partition key: user home shard with replicated public read projections.
4. Draw Privacy-aware feed, profile, and media caches and Graph, content, media, and policy events only after explaining which reads and side effects may be derived or eventual.
5. Separate Fan-out, media, indexing, and deletion workers, Recommendation and bounded personal-agent gateway, and Identity recovery and media processing providers from the authoritative transaction with deadlines and durable events.
...
| Authoritative state | Sharded graph and content databases | Managed relational, distributed SQL, key-value, or dedicated state fleet | Transactions, quorum, latency, partitioning, restore behavior. |
| Fast serving state | Privacy-aware feed, profile, and media caches | In-memory cache, read replica, search/vector index | Staleness, invalidation, hot-key behavior, rebuild time. |
| Durable messaging | Graph, content, media, and policy events | Partitioned log, managed queue, transactional outbox relay | Ordering, replay, retention, throughput, consumer isolation. |
...
| AI/model serving | Recommendation and bounded personal-agent gateway | In-cluster serving or managed inference endpoint | Tail latency, policy, model routing, cost, deterministic fallback. |
