---
title: "System Design Deployment Architecture Worksheet"
summary: "A cloud-neutral deployment using reusable regional-cell deployment worksheet for System under design, with explicit failure domains, state ownership, recovery, and AI degradation."
systemDesign: 00-system-design-template
order: 1
deploymentStyle: "Reusable regional-cell deployment worksheet"
availabilityTarget: "State an explicit availability or durability objective"
regions: multi-region
tags: [deployment-method, failure-domains, capacity, disaster-recovery]
---
...
Use this worksheet to choose the critical invariant, availability objective, ownership boundary, and recovery target before selecting infrastructure. The synchronous customer path remains short; derived views, external calls, analytics, and expensive inference move behind durable boundaries whenever the product contract permits.

Assume three independent failure zones in an active region, immutable artifacts, workload identity, managed or dedicated state services separated from stateless request compute, and a paired recovery region. The interviewer will confirm actors, scale, consistency, geography, retention, and compliance before architecture selection. The Kubernetes or compute API is a management plane and never carries customer requests.

## 2. Traffic classes and critical paths

| Traffic class | Latency or freshness objective | Consistency | Deployment implication |
|---|---|---|---|
| Critical command | State a measurable p99 target for the critical request | Enforce the product invariant before acknowledgement | Ownership-route to zonal service replicas and authoritative regional state. |
...
| Event and background work | Seconds to minutes by business priority | At-least-once with idempotent consumers | Partition Durable event boundary and isolate critical from bulk queues. |
...
```mermaid
sequenceDiagram
  accTitle: System under design critical deployed request path
  ...
  participant S as Critical stateless service tier
  participant D as Chosen system of record
  participant Q as Durable event boundary
  participant W as Async and batch workers
  participant A as Optional model gateway
...
```mermaid
flowchart TB
  accTitle: System under design cloud-neutral deployment
  ...
  Client["Clients and operators<br/>Issue product commands and reads"] --> Edge["Edge protection and traffic policy<br/>Terminates public traffic and rejects abuse"]
  ...
  subgraph Active["Active serving region"]
    LB["Regional load balancer and gateway<br/>Distributes traffic across healthy zones"]
    CP["Managed compute control plane<br/>Schedules and manages workloads only"]
    subgraph Compute["Private workload cluster"]
      ZA["Zone A replicas<br/>Gateway and Critical stateless service tier"]
      ZB["Zone B replicas<br/>Gateway and Critical stateless service tier"]
      ZC["Zone C replicas<br/>Gateway and Critical stateless service tier"]
      Worker["Async and batch workers<br/>Consumes durable retryable work"]
      AI["Optional model gateway<br/>Deadline-bounded and policy-controlled"]
    ...
    subgraph State["Managed or dedicated state plane outside request compute"]
      Primary[("Chosen system of record<br/>Owns authoritative durable state")]
      Fast[("Cache or serving index<br/>Serves low-latency derived or ephemeral state")]
      Stream[("Durable event boundary<br/>Decouples committed asynchronous work")]
  ...
  Traffic --> LB
  Worker --> External["Bounded external dependency<br/>Reached through restricted egress"]
...
|---|---|---|---|---|
| Edge protection and traffic policy | Globally or regionally distributed edge | Edge location and policy | Routing/config only | Rejects attacks and routes only to explicitly healthy regions. |
| Regional load balancer and gateway | Active region across three zones | Managed capacity or gateway replica | No | Drains failed zones and preserves connection or request retries. |
...
| Critical stateless service tier | Private zonal replicas in Private workload cluster | Replica by CPU, concurrency, connections, or queue lag | No local authority | Losing one replica or zone removes endpoints without losing durable state. |
| Chosen system of record | Managed or dedicated multi-zone state plane | Shard, partition, connection, and storage unit | Yes | Quorum or replica failover preserves acknowledged authoritative state. |
| Cache or serving index | Regional replicas or immutable serving indexes | Shard and memory/index bytes | Derived or ephemeral | Rebuilds from authority; stale data is bounded or bypassed for policy changes. |
| Durable event boundary | Multi-zone partitioned durable log/queue | Partition throughput and retention | Yes | Consumers replay from checkpoints after worker failure. |
| Async and batch workers | Private autoscaled worker pools | Queue age, lag, and concurrency | Checkpoints only | Work is retried idempotently or isolated in a dead-letter workflow. |
| Optional model gateway | Isolated CPU/accelerator or managed inference pool | Requests, tokens, batches, and accelerator utilization | Model artifacts | Circuit opens to a deterministic fallback without failing the product. |
...
Edge protection and traffic policy terminates public TLS, applies DDoS/WAF controls, rate limits abuse, and strips untrusted forwarding metadata. Traffic policy chooses a region only after independent health and ownership checks. At Regional load balancer and gateway, the platform authenticates or validates signed identity, attaches trace and idempotency context, enforces request-size and deadline policy, and distributes traffic across healthy zones.
...
Place Private workload cluster across three zones with topology spread, disruption budgets, anti-affinity for critical replicas, and reserved capacity so any one zone can disappear. Run Critical stateless service tier as stateless deployments where the domain permits; give Async and batch workers separate queues and resource pools so backlogs cannot exhaust request capacity. Specialized inference is isolated from product compute.
...
Treat Chosen system of record as authoritative. Partition by the domain ownership key selected during the interview. Apply strong consistency only to Identify the invariant that must never be violated; allow bounded eventual consistency for derived caches, indexes, analytics, and model features. Give Cache or serving index a rebuild path and version watermark so stale values cannot overrule newer security or deletion state.

Begin Durable event boundary after a durable commit, preferably through a transactional outbox or native log append. Consumers checkpoint offsets, deduplicate event IDs, quarantine poison records, and expose oldest-work age. Archives, snapshots, and model artifacts use encrypted versioned object storage with lifecycle policy.

## 7. Network zones and security boundaries

Separate public edge, private application, restricted state, management, telemetry, and controlled-egress zones. Default-deny network policy allows only named service identities and ports. Workload identity replaces static credentials; secrets and envelope keys rotate through managed key services. Classify sensitive data and enforce least-privilege access.

Bounded external dependency is reached through allowlisted egress proxies with deadlines, circuit breakers, response-size limits, and audit. Treat external and model output as untrusted data. Administrative and break-glass actions require strong authentication, least privilege, short sessions, immutable audit, and where consequence demands it, dual approval.
...
The sizing method is explicit: derive replicas from measured per-instance throughput, then add peak, rebalance, and one-zone-loss headroom. Load-test the complete protocol and record shape rather than relying on vendor headline throughput. Keep at least 30% burst/rebalance headroom plus enough reserved capacity to survive the largest declared failure domain.
...
The assumed **RPO** is chosen per data class before selecting replication. The assumed **RTO** is chosen per customer journey and tested by restoration drills. These are product choices, not properties automatically obtained by creating a second cluster. Backups are encrypted, immutable where required, restored in drills, and checked against application-level invariants.
...
|---|---|---|---|
| Failure: one availability zone is lost | zone health and SLO burn | drain endpoints and use reserved capacity in healthy zones | critical requests continue after reconnect |
| Failure: the authoritative store is unavailable | transaction errors and quorum health | stop unsafe writes, preserve idempotency, and serve permitted stale reads | correctness is preserved while writes pause |
| Failure: AI inference is slow or unavailable | deadline, circuit state, and quality guardrail | open the circuit and invoke the deterministic fallback | core product behavior remains available |

AI degradation is explicit: remove inference and return the deterministic product result or a clearly reduced non-AI experience. The **deterministic product fallback** is tested in canaries and game days; inference may improve ranking, prediction, classification, or assistance but cannot bypass identity, authorization, ownership, money, privacy, durability, or safety rules.
...
1. Start at Edge protection and traffic policy: explain trust, routing, and what can be served safely at the edge.
2. Follow the solid request path through Regional load balancer and gateway into zonal Critical stateless service tier replicas.
3. Identify Chosen system of record as authoritative and state the partition key: the domain ownership key selected during the interview.
4. Draw Cache or serving index and Durable event boundary only after explaining which reads and side effects may be derived or eventual.
5. Separate Async and batch workers, Optional model gateway, and Bounded external dependency from the authoritative transaction with deadlines and durable events.
...
| Authoritative state | Chosen system of record | Managed relational, distributed SQL, key-value, or dedicated state fleet | Transactions, quorum, latency, partitioning, restore behavior. |
| Fast serving state | Cache or serving index | In-memory cache, read replica, search/vector index | Staleness, invalidation, hot-key behavior, rebuild time. |
| Durable messaging | Durable event boundary | Partitioned log, managed queue, transactional outbox relay | Ordering, replay, retention, throughput, consumer isolation. |
...
| AI/model serving | Optional model gateway | In-cluster serving or managed inference endpoint | Tail latency, policy, model routing, cost, deterministic fallback. |
