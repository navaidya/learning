---
title: "AI-Native Cloud Knowledge Workspace Deployment Architecture"
summary: "A cloud-neutral deployment using tenant-affine metadata cells with global object durability for Cloud knowledge workspace, with explicit failure domains, state ownership, recovery, and AI degradation."
systemDesign: 03-ai-native-cloud-knowledge-workspace
order: 4
deploymentStyle: "Tenant-affine metadata cells with global object durability"
availabilityTarget: "99.99% metadata availability with eleven-nines object durability target"
regions: multi-region
tags: [object-storage, sync, hybrid-search, tenant-isolation]
---
...
The deployment protects the parent design's critical invariant—ACL checks precede every metadata, search, and grounded-answer result—while meeting 99.99% metadata availability with eleven-nines object durability target. The synchronous customer path remains short; derived views, external calls, analytics, and expensive inference move behind durable boundaries whenever the product contract permits.

Assume three independent failure zones in an active region, immutable artifacts, workload identity, managed or dedicated state services separated from stateless request compute, and a paired recovery region. Large blobs use multipart transfer, metadata is region-affine, and AI search is tenant-authorized and opt-in. The Kubernetes or compute API is a management plane and never carries customer requests.

## 2. Traffic classes and critical paths

| Traffic class | Latency or freshness objective | Consistency | Deployment implication |
|---|---|---|---|
| Critical command | metadata reads p99 below 200 ms and resumable chunk transfer saturates the client link | Enforce the product invariant before acknowledgement | Ownership-route to zonal service replicas and authoritative regional state. |
...
| Event and background work | Seconds to minutes by business priority | At-least-once with idempotent consumers | Partition Version and indexing event stream and isolate critical from bulk queues. |
...
```mermaid
sequenceDiagram
  accTitle: Cloud knowledge workspace critical deployed request path
  ...
  participant S as Metadata, chunk, sharing, and sync services
  participant D as Transactional metadata and ACL database
  participant Q as Version and indexing event stream
  participant W as Chunk, indexing, OCR, and retention workers
  participant A as ACL-filtered retrieval and RAG serving
...
```mermaid
flowchart TB
  accTitle: Cloud knowledge workspace cloud-neutral deployment
  ...
  Client["Clients and operators<br/>Issue product commands and reads"] --> Edge["Global upload acceleration and API protection<br/>Terminates public traffic and rejects abuse"]
  ...
  subgraph Active["Active serving region"]
    LB["Regional sync and metadata gateway<br/>Distributes traffic across healthy zones"]
    CP["Managed compute control plane<br/>Schedules and manages workloads only"]
    subgraph Compute["Tenant-affine application cluster"]
      ZA["Zone A replicas<br/>Gateway and Metadata, chunk, sharing, and sync services"]
      ZB["Zone B replicas<br/>Gateway and Metadata, chunk, sharing, and sync services"]
      ZC["Zone C replicas<br/>Gateway and Metadata, chunk, sharing, and sync services"]
      Worker["Chunk, indexing, OCR, and retention workers<br/>Consumes durable retryable work"]
      AI["ACL-filtered retrieval and RAG serving<br/>Deadline-bounded and policy-controlled"]
    ...
    subgraph State["Managed or dedicated state plane outside request compute"]
      Primary[("Transactional metadata and ACL database<br/>Owns authoritative durable state")]
      Fast[("Change cursors and authorized search indexes<br/>Serves low-latency derived or ephemeral state")]
      Stream[("Version and indexing event stream<br/>Decouples committed asynchronous work")]
...
|---|---|---|---|---|
| Global upload acceleration and API protection | Globally or regionally distributed edge | Edge location and policy | Routing/config only | Rejects attacks and routes only to explicitly healthy regions. |
| Regional sync and metadata gateway | Active region across three zones | Managed capacity or gateway replica | No | Drains failed zones and preserves connection or request retries. |
...
| Metadata, chunk, sharing, and sync services | Private zonal replicas in Tenant-affine application cluster | Replica by CPU, concurrency, connections, or queue lag | No local authority | Losing one replica or zone removes endpoints without losing durable state. |
| Transactional metadata and ACL database | Managed or dedicated multi-zone state plane | Shard, partition, connection, and storage unit | Yes | Quorum or replica failover preserves acknowledged authoritative state. |
| Change cursors and authorized search indexes | Regional replicas or immutable serving indexes | Shard and memory/index bytes | Derived or ephemeral | Rebuilds from authority; stale data is bounded or bypassed for policy changes. |
| Version and indexing event stream | Multi-zone partitioned durable log/queue | Partition throughput and retention | Yes | Consumers replay from checkpoints after worker failure. |
| Chunk, indexing, OCR, and retention workers | Private autoscaled worker pools | Queue age, lag, and concurrency | Checkpoints only | Work is retried idempotently or isolated in a dead-letter workflow. |
| ACL-filtered retrieval and RAG serving | Isolated CPU/accelerator or managed inference pool | Requests, tokens, batches, and accelerator utilization | Model artifacts | Circuit opens to a deterministic fallback without failing the product. |
...
Global upload acceleration and API protection terminates public TLS, applies DDoS/WAF controls, rate limits abuse, and strips untrusted forwarding metadata. Traffic policy chooses a region only after independent health and ownership checks. At Regional sync and metadata gateway, the platform authenticates or validates signed identity, attaches trace and idempotency context, enforces request-size and deadline policy, and distributes traffic across healthy zones.
...
Place Tenant-affine application cluster across three zones with topology spread, disruption budgets, anti-affinity for critical replicas, and reserved capacity so any one zone can disappear. Run Metadata, chunk, sharing, and sync services as stateless deployments where the domain permits; give Chunk, indexing, OCR, and retention workers separate queues and resource pools so backlogs cannot exhaust request capacity. Specialized inference is isolated from product compute.
...
Treat Transactional metadata and ACL database as authoritative. Partition by tenant and namespace root, with immutable chunks addressed by content hash. Apply strong consistency only to ACL checks precede every metadata, search, and grounded-answer result; allow bounded eventual consistency for derived caches, indexes, analytics, and model features. Give Change cursors and authorized search indexes a rebuild path and version watermark so stale values cannot overrule newer security or deletion state.

Begin Version and indexing event stream after a durable commit, preferably through a transactional outbox or native log append. Consumers checkpoint offsets, deduplicate event IDs, quarantine poison records, and expose oldest-work age. Archives, snapshots, and model artifacts use encrypted versioned object storage with lifecycle policy.

## 7. Network zones and security boundaries

Separate public edge, private application, restricted state, management, telemetry, and controlled-egress zones. Default-deny network policy allows only named service identities and ports. Workload identity replaces static credentials; secrets and envelope keys rotate through managed key services. encrypt tenant content, isolate keys, audit access, and prevent cross-tenant retrieval.
...
The sizing method is explicit: metadata QPS sizes stateless services while multipart bandwidth, chunk count, and index lag independently size transfer and worker pools. Load-test the complete protocol and record shape rather than relying on vendor headline throughput. Keep at least 30% burst/rebalance headroom plus enough reserved capacity to survive the largest declared failure domain.
...
The assumed **RPO** is zero for acknowledged metadata and object writes, under 5 minutes for derived indexes. The assumed **RTO** is under 15 minutes for tenant metadata failover and hours for full regional index rebuild. These are product choices, not properties automatically obtained by creating a second cluster. Backups are encrypted, immutable where required, restored in drills, and checked against application-level invariants.
...
|---|---|---|---|
| Failure: one metadata zone fails | database quorum and gateway health | fail over connections and retain tenant affinity in healthy zones | sync briefly retries without namespace divergence |
| Failure: indexing falls behind | change-stream age and index watermark | scale workers and serve last indexed version with freshness label | file sync remains healthy |
| Failure: RAG inference is unavailable | model deadline and circuit state | use the deterministic lexical and metadata-search fallback | users still find and open authorized files |

AI degradation is explicit: return deterministic filename, metadata, and lexical-search results when embeddings or generation are unavailable. The **deterministic product fallback** is tested in canaries and game days; inference may improve ranking, prediction, classification, or assistance but cannot bypass identity, authorization, ownership, money, privacy, durability, or safety rules.
...
1. Start at Global upload acceleration and API protection: explain trust, routing, and what can be served safely at the edge.
2. Follow the solid request path through Regional sync and metadata gateway into zonal Metadata, chunk, sharing, and sync services replicas.
3. Identify Transactional metadata and ACL database as authoritative and state the partition key: tenant and namespace root, with immutable chunks addressed by content hash.
4. Draw Change cursors and authorized search indexes and Version and indexing event stream only after explaining which reads and side effects may be derived or eventual.
5. Separate Chunk, indexing, OCR, and retention workers, ACL-filtered retrieval and RAG serving, and Identity and notification providers from the authoritative transaction with deadlines and durable events.
...
| Authoritative state | Transactional metadata and ACL database | Managed relational, distributed SQL, key-value, or dedicated state fleet | Transactions, quorum, latency, partitioning, restore behavior. |
| Fast serving state | Change cursors and authorized search indexes | In-memory cache, read replica, search/vector index | Staleness, invalidation, hot-key behavior, rebuild time. |
| Durable messaging | Version and indexing event stream | Partitioned log, managed queue, transactional outbox relay | Ordering, replay, retention, throughput, consumer isolation. |
...
| AI/model serving | ACL-filtered retrieval and RAG serving | In-cluster serving or managed inference endpoint | Tail latency, policy, model routing, cost, deterministic fallback. |
