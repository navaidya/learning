---
title: "AI-Native Search Autocomplete Deployment Architecture"
summary: "A cloud-neutral deployment using global edge cache with atomic regional index publication for Search autocomplete service, with explicit failure domains, state ownership, recovery, and AI degradation."
systemDesign: 14-search-autocomplete
order: 15
deploymentStyle: "Global edge cache with atomic regional index publication"
availabilityTarget: "99.99% serving availability with last-known-good index fallback"
regions: global
tags: [edge-cache, indexes, publication, privacy]
---
...
The deployment protects the parent design's critical invariant—suppressed or unsafe terms disappear within the declared invalidation bound—while meeting 99.99% serving availability with last-known-good index fallback. The synchronous customer path remains short; derived views, external calls, analytics, and expensive inference move behind durable boundaries whenever the product contract permits.

Assume three independent failure zones in an active region, immutable artifacts, workload identity, managed or dedicated state services separated from stateless request compute, and a paired recovery region. Indexes are immutable by version, personalization is consent-bounded, and serving never calls a large model synchronously. The Kubernetes or compute API is a management plane and never carries customer requests.

## 2. Traffic classes and critical paths

| Traffic class | Latency or freshness objective | Consistency | Deployment implication |
|---|---|---|---|
| Critical command | p99 suggestion response below 50 ms from the nearest serving region | Enforce the product invariant before acknowledgement | Ownership-route to zonal service replicas and authoritative regional state. |
...
| Event and background work | Seconds to minutes by business priority | At-least-once with idempotent consumers | Partition Trend, entity, and policy update streams and isolate critical from bulk queues. |
...
```mermaid
sequenceDiagram
  accTitle: Search autocomplete service critical deployed read path
  ...
  participant F as Immutable prefix indexes and edge caches
  participant S as Candidate retrieval, filtering, merge, and response services
  participant D as Versioned source and suppression configuration
  participant Q as Trend, entity, and policy update streams
  participant A as Consent-bounded reranking and safety classification
...
```mermaid
flowchart TB
  accTitle: Search autocomplete service cloud-neutral deployment
  ...
  Client["Clients and operators<br/>Issue product commands and reads"] --> Edge["Global suggestion CDN and abuse protection<br/>Terminates public traffic and rejects abuse"]
  ...
  subgraph Active["Active serving region"]
    LB["Regional low-latency suggestion gateway<br/>Distributes traffic across healthy zones"]
    CP["Managed compute control plane<br/>Schedules and manages workloads only"]
    subgraph Compute["Serving replicas plus isolated index-build pools"]
      ZA["Zone A replicas<br/>Gateway and Candidate retrieval, filtering, merge, and response services"]
      ZB["Zone B replicas<br/>Gateway and Candidate retrieval, filtering, merge, and response services"]
      ZC["Zone C replicas<br/>Gateway and Candidate retrieval, filtering, merge, and response services"]
      Worker["Aggregation, build, validation, and publication workers<br/>Consumes durable retryable work"]
      AI["Consent-bounded reranking and safety classification<br/>Deadline-bounded and policy-controlled"]
    ...
    subgraph State["Managed or dedicated state plane outside request compute"]
      Primary[("Versioned source and suppression configuration<br/>Owns authoritative durable state")]
      Fast[("Immutable prefix indexes and edge caches<br/>Serves low-latency derived or ephemeral state")]
      Stream[("Trend, entity, and policy update streams<br/>Decouples committed asynchronous work")]
  ...
  Traffic --> LB
  Worker --> External["Search corpus and privacy/consent service<br/>Reached through restricted egress"]
...
|---|---|---|---|---|
| Global suggestion CDN and abuse protection | Globally or regionally distributed edge | Edge location and policy | Routing/config only | Rejects attacks and routes only to explicitly healthy regions. |
| Regional low-latency suggestion gateway | Active region across three zones | Managed capacity or gateway replica | No | Drains failed zones and preserves connection or request retries. |
...
| Candidate retrieval, filtering, merge, and response services | Private zonal replicas in Serving replicas plus isolated index-build pools | Replica by CPU, concurrency, connections, or queue lag | No local authority | Losing one replica or zone removes endpoints without losing durable state. |
| Versioned source and suppression configuration | Managed or dedicated multi-zone state plane | Shard, partition, connection, and storage unit | Yes | Quorum or replica failover preserves acknowledged authoritative state. |
| Immutable prefix indexes and edge caches | Regional replicas or immutable serving indexes | Shard and memory/index bytes | Derived or ephemeral | Rebuilds from authority; stale data is bounded or bypassed for policy changes. |
| Trend, entity, and policy update streams | Multi-zone partitioned durable log/queue | Partition throughput and retention | Yes | Consumers replay from checkpoints after worker failure. |
| Aggregation, build, validation, and publication workers | Private autoscaled worker pools | Queue age, lag, and concurrency | Checkpoints only | Work is retried idempotently or isolated in a dead-letter workflow. |
| Consent-bounded reranking and safety classification | Isolated CPU/accelerator or managed inference pool | Requests, tokens, batches, and accelerator utilization | Model artifacts | Circuit opens to a deterministic fallback without failing the product. |
...
Global suggestion CDN and abuse protection terminates public TLS, applies DDoS/WAF controls, rate limits abuse, and strips untrusted forwarding metadata. Traffic policy chooses a region only after independent health and ownership checks. At Regional low-latency suggestion gateway, the platform authenticates or validates signed identity, attaches trace and idempotency context, enforces request-size and deadline policy, and distributes traffic across healthy zones.
...
Place Serving replicas plus isolated index-build pools across three zones with topology spread, disruption budgets, anti-affinity for critical replicas, and reserved capacity so any one zone can disappear. Run Candidate retrieval, filtering, merge, and response services as stateless deployments where the domain permits; give Aggregation, build, validation, and publication workers separate queues and resource pools so backlogs cannot exhaust request capacity. Specialized inference is isolated from product compute.
...
Treat Versioned source and suppression configuration as authoritative. Partition by language and normalized prefix, with replicated hot-prefix ranges. Apply strong consistency only to suppressed or unsafe terms disappear within the declared invalidation bound; allow bounded eventual consistency for derived caches, indexes, analytics, and model features. Give Immutable prefix indexes and edge caches a rebuild path and version watermark so stale values cannot overrule newer security or deletion state.

Begin Trend, entity, and policy update streams after a durable commit, preferably through a transactional outbox or native log append. Consumers checkpoint offsets, deduplicate event IDs, quarantine poison records, and expose oldest-work age. Archives, snapshots, and model artifacts use encrypted versioned object storage with lifecycle policy.

## 7. Network zones and security boundaries

Separate public edge, private application, restricted state, management, telemetry, and controlled-egress zones. Default-deny network policy allows only named service identities and ports. Workload identity replaces static credentials; secrets and envelope keys rotate through managed key services. minimize raw query retention, protect children and sensitive terms, and prevent trend manipulation.

Search corpus and privacy/consent service is reached through allowlisted egress proxies with deadlines, circuit breakers, response-size limits, and audit. Treat external and model output as untrusted data. Administrative and break-glass actions require strong authentication, least privilege, short sessions, immutable audit, and where consequence demands it, dual approval.
...
The sizing method is explicit: requests/s times keystrokes size edge and serving replicas while index bytes, build frequency, and publication fan-out size builders. Load-test the complete protocol and record shape rather than relying on vendor headline throughput. Keep at least 30% burst/rebalance headroom plus enough reserved capacity to survive the largest declared failure domain.
...
The assumed **RPO** is zero for suppression policy and under one build interval for popularity signals. The assumed **RTO** is seconds by pinning last-known-good index and minutes to republish a regional version. These are product choices, not properties automatically obtained by creating a second cluster. Backups are encrypted, immutable where required, restored in drills, and checked against application-level invariants.
...
|---|---|---|---|
| Failure: a new index is corrupt | validation, checksum, and canary metrics | retain the last-known-good pointer and reject publication | suggestions continue on the prior version |
| Failure: a hot prefix overloads a shard | per-prefix QPS and tail latency | edge-cache, replicate, and request-coalesce the range | other languages and prefixes remain isolated |
| Failure: reranking inference fails | deadline, consent, or budget guardrail | use deterministic prefix popularity fallback | autocomplete remains within latency budget |

AI degradation is explicit: serve deterministic prefix popularity and policy-filtered results when personalization or models are unavailable. The **deterministic product fallback** is tested in canaries and game days; inference may improve ranking, prediction, classification, or assistance but cannot bypass identity, authorization, ownership, money, privacy, durability, or safety rules.
...
1. Start at Global suggestion CDN and abuse protection: explain trust, routing, and what can be served safely at the edge.
2. Follow the solid request path through Regional low-latency suggestion gateway into zonal Candidate retrieval, filtering, merge, and response services replicas.
3. Identify Versioned source and suppression configuration as authoritative and state the partition key: language and normalized prefix, with replicated hot-prefix ranges.
4. Draw Immutable prefix indexes and edge caches and Trend, entity, and policy update streams only after explaining which reads and side effects may be derived or eventual.
5. Separate Aggregation, build, validation, and publication workers, Consent-bounded reranking and safety classification, and Search corpus and privacy/consent service from the authoritative transaction with deadlines and durable events.
...
| Authoritative state | Versioned source and suppression configuration | Managed relational, distributed SQL, key-value, or dedicated state fleet | Transactions, quorum, latency, partitioning, restore behavior. |
| Fast serving state | Immutable prefix indexes and edge caches | In-memory cache, read replica, search/vector index | Staleness, invalidation, hot-key behavior, rebuild time. |
| Durable messaging | Trend, entity, and policy update streams | Partitioned log, managed queue, transactional outbox relay | Ordering, replay, retention, throughput, consumer isolation. |
...
| AI/model serving | Consent-bounded reranking and safety classification | In-cluster serving or managed inference endpoint | Tail latency, policy, model routing, cost, deterministic fallback. |
