...
Run Java: `cd java && rm -rf out && javac -d out $(find src -name '*.java') && java -ea -cp out BoundedQueueTest`. Run Python: `cd python && python3 -m unittest discover -s tests -v`. Expected result: Java exits 0 with no output; Python reports all tests passing.
